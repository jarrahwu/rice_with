...
public class ViewBinder<T extends IBind<?>> {

	private static final boolean DEBUG = Constant.DBG;
	...
	public void bindClicks() {
		for (Field field : getViewBindFields()) {
			...
			if (methodName != Bind.NO_CLICK && id != Bind.NO_ID) {
				if (DEBUG) {
...
//		Util.e("declared bindField length", getViewBindFields().length);
		for (Field f : getViewBindFields()) {
			int id = f.getAnnotation(Bind.class).id();
			if (id != Bind.NO_ID) {
